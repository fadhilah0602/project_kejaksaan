...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['id_user']) && isset($_POST['laporan']) && isset($_POST['ktp']) && isset($_POST['nama_pelapor']) && isset($_POST['no_hp'])) {
        ...
        if (!empty($_FILES['ktp_pdf']['name'])) {
            ...
            $sql = "SELECT ktp_pdf FROM pengaduan WHERE id_pengaduan = '$id_pengaduan'";
            $result = $koneksi->query($sql);
            if ($result->num_rows > 0) {
                $row = $result->fetch_assoc();
                $ktp_pdf = $row['ktp_pdf'];
        ...
        $sql = "INSERT INTO pengaduan (id_user, laporan_pdf, ktp_pdf, laporan, ktp, nama_pelapor, no_hp, status) 
        VALUES ('$id_user', '$laporan_pdf', '$ktp_pdf', '$laporan', '$ktp', '$nama_pelapor', '$no_hp', '$status')";

        if ($koneksi->query($sql) === TRUE) {
            $response['isSuccess'] = true;
            $response['message'] = "Berhasil manambah data pengaduan";
        } else {
            $response['isSuccess'] = false;
            $response['message'] = "Gagal menambah data pengaduan: " . $koneksi->error;
        }
    } else {
        $response['isSuccess'] = false;
