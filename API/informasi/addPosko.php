...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['id_user']) && isset($_POST['laporan']) && isset($_POST['ktp']) && isset($_POST['nama_pelapor']) && isset($_POST['no_hp'])) {
        ...
        $sql = "INSERT INTO posko (id_user, laporan_pdf, ktp_pdf, laporan, ktp, nama_pelapor, no_hp, status) 
        VALUES ('$id_user', '$laporan_pdf', '$ktp_pdf', '$laporan', '$ktp', '$nama_pelapor', '$no_hp', '$status')";

        if ($koneksi->query($sql) === TRUE) {
            $response['isSuccess'] = true;
            $response['message'] = "Berhasil manambah data posko pilkada";
        } else {
            $response['isSuccess'] = false;
            $response['message'] = "Gagal menambah data posko pilkada: " . $koneksi->error;
        }
    } else {
        $response['isSuccess'] = false;
