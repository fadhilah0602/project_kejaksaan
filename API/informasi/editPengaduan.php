...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['id_pengaduan']) && isset($_POST['id_user']) && isset($_POST['laporan']) && isset($_POST['ktp']) && isset($_POST['nama_pelapor']) && isset($_POST['no_hp'])) {
        ...
        if (!empty($_FILES['laporan_pdf']['name'])) {
            ...
            $sql = "SELECT laporan_pdf FROM pengaduan WHERE id_pengaduan = '$id_pengaduan'";
        ...
        if (!empty($_FILES['ktp_pdf']['name'])) {
            ...
            $sql = "SELECT ktp_pdf FROM pengduan WHERE id_pengaduan = '$id_pengaduan'";
            $result = $koneksi->query($sql);
            if ($result->num_rows > 0) {
                $row = $result->fetch_assoc();
                $ktp_pdf = $row['ktp_pdf'];
        ...
        $sql = "UPDATE pengaduan SET id_user='$id_user', laporan_pdf='$laporan_pdf', ktp_pdf='$ktp_pdf', laporan='$laporan', ktp='$ktp', nama_pelapor='$nama_pelapor', no_hp='$no_hp', status='$status' WHERE id_pengaduan='$id_pengaduan'";
