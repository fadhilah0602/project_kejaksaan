...
import 'list_posko_customers.dart';

class EditPoskoRejectedScreen extends StatefulWidget {
  final ModelPosko posko;

  const EditPoskoRejectedScreen({Key? key, required this.posko})
      : super(key: key);

  @override
  State<EditPoskoRejectedScreen> createState() => _EditPoskoRejectedScreenState();
}

class _EditPoskoRejectedScreenState extends State<EditPoskoRejectedScreen> {
  ...
  void initState() {
    super.initState();
    // _loadUserId();
    _laporanController.text = widget.posko.data.first.laporan ?? '';
    _namaPelaporController.text = widget.posko.data.first.nama_pelapor ?? '';
    _ktpController.text = widget.posko.data.first.ktp ?? '';
    _noHpController.text = widget.posko.data.first.no_hp ?? '';
  ...
  Future<void> _updatePengaduan() async {
    if (_formKey.currentState!.validate()) {
      try {
        Uri uri = Uri.parse('http://192.168.42.183/informasi/editPosko.php');

        http.MultipartRequest request = http.MultipartRequest('POST', uri)
          ..fields['id_posko'] = widget.posko.data.first.id_posko
          ..fields['id_user'] = widget.posko.data.first.id_user
          ...
          ..fields['status'] = widget.posko.data.first.status; // Set the status field
        ...
        if (response.statusCode == 200) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => ListPoskoCustomersScreen()),
          );
        }
  ...
  Future<void> deleteJms() async {
    Uri uri = Uri.parse('http://192.168.42.183/informasi/deletePosko.php');
    http.MultipartRequest request = http.MultipartRequest('POST', uri)
      ..fields['id_posko'] = widget.posko.data.first.id_posko;
