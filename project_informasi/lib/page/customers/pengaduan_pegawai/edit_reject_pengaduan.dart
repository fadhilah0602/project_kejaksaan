...
import 'list_pengaduan_customers.dart';

class EditPengaduanRejectedScreen extends StatefulWidget {
  final ModelPengaduan pengaduan;

  const EditPengaduanRejectedScreen({Key? key, required this.pengaduan})
      : super(key: key);

  @override
  State<EditPengaduanRejectedScreen> createState() => _EditPengaduanRejectedScreenState();
}

class _EditPengaduanRejectedScreenState extends State<EditPengaduanRejectedScreen> {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => ListPengaduanCustomersScreen()),
          );
        }
  ...
  Future<void> deleteJms() async {
    Uri uri = Uri.parse('http://192.168.42.183/informasi/deletePengaduan.php');
    http.MultipartRequest request = http.MultipartRequest('POST', uri)
      ..fields['id_pengaduan'] = widget.pengaduan.data.first.id_pengaduan;
