...
import 'list_pengawasan_customers.dart';

class EditPengawasanRejectedScreen extends StatefulWidget {
  final ModelPengawasan pengawasan;

  const EditPengawasanRejectedScreen({Key? key, required this.pengawasan})
      : super(key: key);

  @override
  State<EditPengawasanRejectedScreen> createState() => _EditPengawasanRejectedScreenState();
}

class _EditPengawasanRejectedScreenState extends State<EditPengawasanRejectedScreen> {
  ...
  void initState() {
    super.initState();
    // _loadUserId();
    _laporanController.text = widget.pengawasan.data.first.laporan ?? '';
    _namaPelaporController.text = widget.pengawasan.data.first.nama_pelapor ?? '';
    _ktpController.text = widget.pengawasan.data.first.ktp ?? '';
    _noHpController.text = widget.pengawasan.data.first.no_hp ?? '';
  ...
  Future<void> _updatePengaduan() async {
    if (_formKey.currentState!.validate()) {
      try {
        Uri uri = Uri.parse('http://192.168.42.183/informasi/editPengawasan.php');

        http.MultipartRequest request = http.MultipartRequest('POST', uri)
          ..fields['id_pengawasan'] = widget.pengawasan.data.first.id_pengawasan
          ..fields['id_user'] = widget.pengawasan.data.first.id_user
          ...
          ..fields['status'] = widget.pengawasan.data.first.status; // Set the status field
        ...
        if (response.statusCode == 200) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => ListPengawasanCustomersScreen()),
          );
        }
  ...
  Future<void> deleteJms() async {
    Uri uri = Uri.parse('http://192.168.42.183/informasi/deletePengawasan.php');
    http.MultipartRequest request = http.MultipartRequest('POST', uri)
      ..fields['id_pengawasan'] = widget.pengawasan.data.first.id_pengawasan;
