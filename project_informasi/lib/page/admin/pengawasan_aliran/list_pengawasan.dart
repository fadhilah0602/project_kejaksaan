...
import 'edit_pengawasan_admin.dart';

class ListPengawasanScreen extends StatefulWidget {
  const ListPengawasanScreen({super.key});

  @override
  State<ListPengawasanScreen> createState() => _ListPengawasanScreenState();
}

class _ListPengawasanScreenState extends State<ListPengawasanScreen> {
  late List<Pengawasan> _pengawasanList;
  late List<Pengawasan> _filteredPengawasanList;
  ...
  Future<void> _fetchPengawasan() async {
    final response = await http
        .get(Uri.parse('http://192.168.42.183/informasi/pengawasan.php'));
    if (response.statusCode == 200) {
      final parsed = jsonDecode(response.body);
      setState(() {
        _pengawasanList =
        List<Pengawasan>.from(parsed['data'].map((x) => Pengawasan.fromJson(x)));
        _filteredPengawasanList = _pengawasanList;
        _isLoading = false;
      });
    } else {
                  ...
                  Container(
                    ...
                    child: Padding(
                      ...
                      child: ListView.builder(
                        ...
                        itemCount: _filteredPengawasanList.length,
                        itemBuilder: (context, index) {
                          final pengawasan = _filteredPengawasanList[index];
                          return Card(
                            child: ListTile(
                                  ...
                                  IconButton(
                                    onPressed: () {
                                      ...
                                      Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (context) => EditPengawasanAdmin(
