...
class EditPoskoAdmin extends StatefulWidget {
  ...
  const EditPoskoAdmin({Key? key, required this.posko})
      : super(key: key);
...
class _EditPoskoAdminState extends State<EditPoskoAdmin> {
  ...
  void initState() {
    ...
    _ktpController = TextEditingController(text: widget.posko.data[0].ktp);
    _ktp_pdfController =
    ...
    _statusValue = widget.posko.data.first.status;
    // _statusValue =
  ...
  //   String url = 'http://192.168.42.183/informasi/posko.php?file=$fileName';
  //   try {
  //     var dio = Dio();
  ...
  Future<void> saveChanges(String newNamaPelapor, String newNohp, String newKtp, String newKtpPdf, String newLaporan,  String newLaporanPdf, String newStatus) async {
    ...
    Uri uri = Uri.parse('http://192.168.42.183/informasi/updateAdminPosko.php');
