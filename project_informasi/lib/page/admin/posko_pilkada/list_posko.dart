...
import 'edit_posko_admin.dart';

class ListPoskoScreen extends StatefulWidget {
  const ListPoskoScreen({super.key});

  @override
  State<ListPoskoScreen> createState() => _ListPoskoScreenState();
}

class _ListPoskoScreenState extends State<ListPoskoScreen> {
  late List<Posko> _poskoList;
  late List<Posko> _filteredPoskoList;
  ...
  Future<void> _fetchPosko() async {
    final response = await http
        .get(Uri.parse('http://192.168.42.183/informasi/posko.php'));
    if (response.statusCode == 200) {
      final parsed = jsonDecode(response.body);
      setState(() {
        _poskoList =
        List<Posko>.from(parsed['data'].map((x) => Posko.fromJson(x)));
        _filteredPoskoList = _poskoList;
        _isLoading = false;
      });
    } else {
            ...
            Positioned(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  labelText: 'Search Posko Pilkada',
                  ...
                  Container(
                    ...
                    child: Padding(
                      ...
                      child: ListView.builder(
                        ...
                        itemCount: _filteredPoskoList.length,
                        itemBuilder: (context, index) {
                          final posko = _filteredPoskoList[index];
                          return Card(
                            child: ListTile(
                                  ...
                                  IconButton(
                                    onPressed: () {
                                      ...
                                      Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (context) => EditPoskoAdmin(
                                              posko: modelPosko),
                                        ),
                                      );
                                    },
